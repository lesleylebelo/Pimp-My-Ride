...
export function registrationErrors(form, role, step) {
 ...
 if (step === 1) {
  ...
  else if(form.fullName.trim().length>200)e.fullName='Use no more than 200 characters.';
 ...
 if (role === 'shop' && step === 2) {
  for (const key of ['shopName','regNumber','address','city','province','services']) {
   if (!form[key]?.trim()) e[key] = 'This field is required.';
   else if(form[key].trim().length>500)e[key]='Use no more than 500 characters.';
  }
 }
