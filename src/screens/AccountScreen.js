...
export default function AccountScreen() {
 const {user,profile,error,refresh,logout}=useAuth();
 ...
 <Text style={{fontSize:26,fontWeight:'800',color:'#0E3B2C'}}>{titles[stage]}</Text>
 ...
 {stage==='verify' && <><Text>Open the verification email sent to {user.email}, then tap “I’ve verified my email”.</Text>
 <PrimaryButton title="I’ve verified my email" loading={busy} onPress={()=>action(refresh)}/>
 <PrimaryButton title="Resend verification email" disabled={busy} onPress={()=>action(async()=>{if(Date.now()<resendAt)throw new Error('Please wait one minute before resending.');await sendEmailVerification(firebase().auth.currentUser);setResendAt(Date.now()+60000);},'Verification email sent.')}/></>}
 ...
 {stage==='incomplete' && !error && <ProfileRecovery/>}
 ...
 {profile?.role==='shop' && profile.verificationStatus==='pending' && <View style={{gap:12}}>
